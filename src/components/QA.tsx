	...
	return (
		<div className="space-y-4">
			...
			<div className="glass-card p-4 overflow-hidden">
				...
				<div
					className={`mt-2 text-white overflow-hidden transition-all duration-300 ease-in-out ${
						openIndex === 0 ? "max-h-48 opacity-100" : "max-h-0 opacity-0"
					}`}>
					Chain Message
					是一个基于区块链的留言系统，你的留言将被永久存储在区块链上，而传统评论系统通常依赖于中心化服务器，这意味着他人可能删除或者篡改你的留言。
				</div>
			</div>
			...
			<div className="glass-card p-4 overflow-hidden">
				...
				<div
					className={`mt-2 text-white overflow-hidden transition-all duration-300 ease-in-out ${
						openIndex === 1 ? "max-h-48 opacity-100" : "max-h-0 opacity-0"
					}`}>
					<strong>
						由于Chain
						Message基于区块链技术，一旦留言被发布，就无法被删除或修改，哪怕是你或者我。
					</strong>
					即使我下线了网站，在区块链上仍然会保留你的留言记录。因此，在留言前请认真确认。
				</div>
			</div>
			...
			<div className="glass-card p-4 overflow-hidden">
				...
				<div
					className={`mt-2 text-white overflow-hidden transition-all duration-300 ease-in-out ${
						openIndex === 2 ? "max-h-48 opacity-100" : "max-h-0 opacity-0"
					}`}>
					每次留言需要支付少量的Gas费用，具体金额取决于当前网络拥堵情况，你可以在交互的时候通过钱包插件查看。在我测试的情况下，大约折合人民币4毛钱。
				</div>
			</div>
			...
			<div className="glass-card p-4 overflow-hidden">
				...
				<div
					className={`mt-2 text-white overflow-hidden transition-all duration-300 ease-in-out ${
						openIndex === 3 ? "max-h-48 opacity-100" : "max-h-0 opacity-0"
					}`}>
			...
			<div className="glass-card p-4 overflow-hidden">
				<button
					...
					<FontAwesomeIcon
						...
						className={`ml-2 transform transition-transform duration-300 ${
							openIndex === 4 ? "rotate-180" : "rotate-0"
						}`}
					/>
				</button>
				<div
					className={`mt-2 text-white overflow-hidden transition-all duration-300 ease-in-out ${
						openIndex === 4 ? "max-h-48 opacity-100" : "max-h-0 opacity-0"
					}`}>
					你可以在 Chain Message 的 Github
					仓库中找到所有源代码（包括前端网站、Cloudflare
					Workers和智能合约）和部署指南：
					<a
						href="https://github.com/shiquda/chain-message"
